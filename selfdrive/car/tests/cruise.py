  ...
  def _update_cruise_buttons(self, CS, CC, v_cruise_kph):
    ...
    if not long_pressed:
      if button_type == ButtonType.accelCruise:
        ...
        if CC.enabled:
          # Pause cruise while preserving the current set speed.
          ...
          # On newer Hyundai models the center button is pause/resume.
          # Resume only when the previous pause was caused by this button.
          self._cruise_cancel_state = False
          self.autoCruiseControl_cancel_timer = 0
          self._lat_enabled = True
